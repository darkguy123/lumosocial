...
class ProfileVerificationController extends ProfilePictureController {
  ...
  void restorePurchase() async {
    startLoading();
    var status = await SubscriptionManager.shared.restorePurchase();
    if (status == true) {
  ...
  void submitRequest() async {
    if (selectedMethod.method == VerificationMethod.document) {
      if (fullNameController.text.isEmpty) {
        ...
        startLoading();
        UserService.shared.profileVerification(fullNameController.text, selectedType?.title ?? '', selectedDocument, XFile(imagePath));
      }
    } else {
